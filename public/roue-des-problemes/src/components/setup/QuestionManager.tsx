    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-white/30 text-xs">
          💬 Ajoutez des questions drôles, pas blessantes. Le groupe répond
          oralement.
        </p>
      </div>
        ...
        <div className="space-y-2">
          ...
          {EXAMPLES.map((ex, i) => (
            <button
              key={i}
              onClick={() => setInput(ex)}
              className="w-full text-left text-xs text-white/40 hover:text-white/70 bg-white/4 hover:bg-white/8 border border-white/6 rounded-lg px-3 py-2 transition-all cursor-pointer"
            >
              {ex}
            </button>
          ))}
          ...
          <AnimatePresence>
            {questions.map((q) => (
              <motion.div
                ...
                className="bg-white/6 border border-white/8 rounded-xl p-3"
              >
                {editingId === q.id ? (
                  <div className="flex gap-2">
                    <textarea
                      ...
                      rows={2}
                      autoFocus
                      className="flex-1 bg-slate-800 border border-violet-400/60 rounded-lg px-3 py-1.5 text-white text-sm focus:outline-none resize-none"
                    />
                  ...
                  <div className="flex items-start gap-2">
                    <span className="flex-1 text-white/80 text-sm leading-snug">
                      {q.text}
                    </span>
                    <div className="flex gap-1 flex-shrink-0">
                      <button
                        onClick={() => startEdit(q)}
                        className="text-white/25 hover:text-violet-400 transition-colors text-sm cursor-pointer"
                        title="Modifier"
                      >
                        ✏️
                      </button>
                      <button
                        ...
                        className="text-white/25 hover:text-red-400 transition-colors text-lg leading-none cursor-pointer"
                        title="Supprimer"
                      >
                        ×
                      </button>
