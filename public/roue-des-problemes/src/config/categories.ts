...
export interface CategoryConfig {
  ...
  label: string
  emoji: string
  /** Total fraction of the wheel (0–1). The wheel splits this into 2 segments. */
  fraction: number
  /** Two shades used for the two wheel segments: [primary, darker] */
  wheelColors: [string, string]
  glow: string
  wheelTextColor: string
  // Tailwind classes for the card UI
  gradient: string
  border: string
  badge: string
  textAccent: string
 ...
 * To add a new category: add one entry here + add its data in useGameState.ts.
 * The wheel, card UI, and history all derive their config from this array.
 */
