...
export default function SpaceInvaders() {
  ...
  // Remplacer le composant EnemyComponent par cette version améliorée qui utilise des fallbacks
  const EnemyComponent = ({ enemy }: { enemy: Enemy }) => {
    ...
    const imageSrc =
      enemy.type === "green"
        ? "/images/green-alien.png"
        : enemy.type === "purple"
          ? "/images/purple-alien.png"
          : "/images/red-stitch.png"
    ...
    return (
      ...
      >
        <Image
          ...
          width={40}
          height={40}
          priority={true} // Priorité de chargement élevée
          style={{
            objectFit: "contain",
            width: "100%",
            height: "100%",
            filter:
              enemy.type === "green"
                ? "drop-shadow(0 0 2px rgba(144, 238, 144, 0.5))"
                : enemy.type === "purple"
                  ? "drop-shadow(0 0 2px rgba(147, 112, 219, 0.5))"
                  : "drop-shadow(0 0 2px rgba(255, 107, 107, 0.5))",
          }}
          onError={(e) => {
            // En cas d'erreur, l'élément div parent avec la couleur de fond sera visible
