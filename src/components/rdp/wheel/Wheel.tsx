...
import type { WheelCategory, WheelPhase } from '@/game/types'
import { CATEGORIES } from '@/game/config/categories'
import {
  SPIN_DURATION_BASE, SPIN_DURATION_EXTRA, SPIN_MIN_ROTATIONS,
  WHEEL_GAP_RADIANS, WHEEL_SIZE_MIN, WHEEL_SIZE_MAX,
...
function buildSegments(): WheelSegment[] {
  const half = CATEGORIES.map((cat, i) => ({
    ...
    fraction: cat.fraction / 2, color: cat.wheelColors[i % 2],
    glow: cat.glow, textColor: cat.wheelTextColor,
  }))
  const second = CATEGORIES.map((cat, i) => ({
    id: cat.id, label: cat.label, emoji: cat.emoji,
    fraction: cat.fraction / 2, color: cat.wheelColors[(i + 1) % 2],
    glow: cat.glow, textColor: cat.wheelTextColor,
  }))
  return [...half, ...second]
}
...
function drawWheel(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, rotation: number) {
...
export function RdpWheel({ phase, initialRotation, onSpinStart, onSpinComplete, disabled = false }: WheelProps) {
  ...
  useEffect(() => {
    const updateSize = () => {
      setSize(Math.min(WHEEL_SIZE_MAX, Math.max(WHEEL_SIZE_MIN, window.innerWidth - 80)))
    }
    updateSize()
  ...
  const redraw = useCallback((rot: number) => {
    ...
    drawWheel(ctx, (size * dpr) / 2, (size * dpr) / 2, (size * dpr) / 2 - 20 * dpr, rot)
    drawPointer(ctx, (size * dpr) / 2, (size * dpr) / 2, (size * dpr) / 2 - 20 * dpr)
  }, [size])
  ...
  const spin = useCallback(() => {
    ...
    onSpinStart()
    const totalRotation = SPIN_MIN_ROTATIONS * Math.PI * 2 + Math.random() * Math.PI * 2
  ...
  const winCat = winningSegment ? CATEGORIES.find((c) => c.id === winningSegment) : null

  return (
    <div className="flex flex-col items-center gap-6">
      <div className="relative">
        <motion.div
          animate={isSpinning ? { scale: [1, 1.08, 1], opacity: [0.4, 0.8, 0.4] } : { scale: 1, opacity: 0.3 }}
          transition={isSpinning ? { duration: 0.8, repeat: Infinity } : { duration: 0.3 }}
          className="absolute inset-0 rounded-full blur-2xl"
          style={{ background: winCat ? `radial-gradient(circle, ${winCat.glow} 0%, transparent 70%)` : 'radial-gradient(circle, rgba(139,92,246,0.4) 0%, transparent 70%)' }}
        />
        <canvas
          ref={canvasRef}
      ...
      {phase === 'idle' && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <RdpButton onClick={spin} disabled={isSpinning || disabled} size="xl" variant="primary">
            🎡 Lancer la roue !
          </RdpButton>
        </motion.div>
      )}
      {phase === 'spinning' && (
        <motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 1, repeat: Infinity }} className="text-white/60 text-sm font-medium">
          La roue tourne…
        </motion.div>
      )}
      {phase === 'result' && winCat && (
        <motion.div
          ...
          style={{ background: `${winCat.wheelColors[0]}33`, border: `2px solid ${winCat.wheelColors[0]}66`, color: winCat.wheelTextColor }}
        >
